...
SELECT *,
AVG(valor_diaria) AS valor_medio_consulta  
FROM consulta c
LEFT JOIN convenio co ON c.id_convenio = co.id_convenio
WHERE YEAR(c.data_hora) = 2020 OR c.id_convenio IS NOT NULL
GROUP BY c.id_consulta;

-- Todas as internações que tiveram data de alta maior que a data prevista para a alta
...
WHERE r.id_receita = (
    SELECT MIN(id_receita) FROM receita
);
...
SELECT i.*, q.numero AS numero_quarto, q.descricao AS tipo_quarto,
q.valor_diaria * DATEDIFF(i.data_efetiva_alta, i.data_entrada) AS total_internacao
FROM internacao i
JOIN quarto q ON i.quarto_paciente = q.id_quarto;

/* Data, procedimento e número de quarto de internações em quartos do tipo “apartamento”. */
SELECT i.data_entrada, i.descricao_procedimento, q.numero AS numero_quarto
FROM internacao i
JOIN quarto q ON i.quarto_paciente = q.id_quarto
...
eram menores de 18 anos na data da consulta e cuja especialidade não seja “pediatria”, ordenando 
por data de realização da consulta. */
SELECT p.nome AS paciente, c.data_hora AS data_consulta, e.nome_especialidade
FROM consulta c
JOIN paciente p ON c.CPF_paciente = p.CPF
JOIN especialidade e ON c.id_especialidade = e.id_especialidade
WHERE TIMESTAMPDIFF(YEAR, p.dtNasc, c.data_hora) < 18
...
SELECT p.nome AS paciente, m.nome AS medico, i.data_entrada, i.descricao_procedimento, q.numero AS numero_quarto
FROM internacao i
JOIN medico m ON i.medico_CRM = m.CRM
JOIN medico_especialidade me ON m.id_medico = me.id_medico
JOIN especialidade e ON me.id_especialidade = e.id_especialidade
JOIN quarto q ON i.quarto_paciente = q.id_quarto
JOIN tipo_quarto tq ON q.tipo = tq.id_tipo
...
-- não tinha nenhum Gabriel, sorry teacher gabs, mas já atualizei 
INSERT INTO medico (CRM, nome, telefone, email, id_convenio) VALUES
('1011', 'Dr. Gabriel B. Jordan', '(11)99952001', 'gabriel.jordan@hosp.com', 1);

SELECT * FROM medico
WHERE nome LIKE '%Gabriel%';

/* Os nomes, CREs e número de internações de enfermeiros que participaram de mais de uma internação. */
SELECT e.nome, e.COREN, COUNT(ie.id_internacao) AS total_internacoes
FROM enfermeiro e
JOIN internacao_enfermeiro ie ON e.id_enfermeiro = ie.id_enfermeiro
